...
## 5.1 Env SSOT And No Fallbacks

**Environment variables must fail fast from the single source of truth.**

- Use root `.env.local` as the single local env source of truth.
- Do not introduce any additional env file besides root `.env.local` unless the user explicitly requests it.
- Do not add fallback values when reading environment variables.
- If `process.env.SOME_KEY` is missing, throw a clear missing environment variable error.
- Do not hide env loading failures with defaults, inferred URLs, or helper fallbacks.
- Before touching env handling, remind yourself that fallback values make configuration errors harder to find.
...
## 7. Plan + Single Checklist

**Before any non-trivial task, use the single project checklist. Don't create duplicate tracking files.**

- **Plan** — what we're building and why.
- **Checklist** — use only `design/checklist.md` as the project checklist. Tick as you go.

Do not create duplicate tracking files. Keep task progress and decisions in `design/checklist.md` when tracking is needed.
