        ...
        messages: [
          {
            ...
            content: [
              ...
              "**GFM 구조(강조):** `#` / `##` / `###`으로 **섹션을 나누고**, 빈 줄로 호흡을 준다. **ATX 제목 줄에는 제목 텍스트만** 둔다. `# 제목` 같은 행 끝에 백틱·본문·괄호 문장을 **같은 줄에 이어 붙이지 않는다**(마크다운이 깨짐). 제목 다음 줄부터 본문·인라인 코드를 시작한다. 설명과 fenced 블록 사이·긴 문단 중간에도 `<br />`로 환기한다. 예시·Before/After·패치는 **적절히 fenced 코드블록**을 쓴다.",
              ...
              "**각 lineComment:** 결함이면 시뮬·분기·기대≠실제·4가지·원인↔반례↔수정·근거·가독성·톤·GFM·**코드는 백틱(식 통째·조사 밖·괄호 공백 없음)·`#`/`##`/`###` 줄은 제목만 단독 행**·**`<br />`로 단계 환기**·**볼드로 핵심 강조**·**괄호 남발 금지**(위 근거·body). **개선 제안만**이면 복잡도·알고리즘 절(정말 유의미·중학생 수준·백틱·점근 근거). 하나라도 미달이면 **그 코멘트 미출력**.",
            ].join("\n"),
          },
          {
            role: "user",
            content: [
              ...
              "body는 `#`/`##`/`###`·코드블록·`<br />`로 **가독성** 챙기고, 중학생 수준으로 풀어 쓰며, 제안체로 **사용자를 배려**해 줄 것. `#`/`##`/`###` 제목 줄에는 제목만 쓰고 같은 줄에 본문이나 백틱을 붙이지 않는다.",
              "본문에 나오는 코드·식·함수명·변수명·리터럴은 모두 백틱으로 감싼다. 실행 과정은 한 줄에 몰아 쓰지 말고 문장으로 풀어 `<br />`로 단계를 나눈다.",
              "fenced 코드 블록: 시작 줄·종료 줄은 **한 줄에 백틱 펜스만**(앞 공백 최대 3칸). `패치 예시: ```javascript`처럼 설명과 같은 줄에 ```를 붙이지 않는다(펜스가 인식되지 않음).",
