        ...
        messages: [
          {
            ...
            content: [
              ...
              "lineComments가 1개 이상이면 summary는 **한 문장 이하**(안내 한 줄, 예: 아래 라인 코멘트 참고). lineComments가 비면 summary는 **긍정·관찰만**; '문제·개선 필요·주의·여지' 등 결함 암시 단어 금지.",
              "매 실행 summary 1개.",
              "",
              "── 결함으로 인정 (4가지만) ──",
              ...
              "",
              "── 복잡도·알고리즘 (선택, 권장) ──",
              "오류 지적과 별개로, **문제가 복잡하고** 명세·제한(입력 범위 등)을 근거로 **시간·공간 복잡도를 줄이는 더 나은 알고리즘·자료구조**가 분명하면, **왜 나은지(점근·대략적 비교)**를 붙여 lineComment로 **권장 피드백**을 줄 수 있다. 매 실행마다 넣을 **의무는 없음**.",
              "정확성 결함이 없어도 위 조건을 만족하면 **개선 제안만**의 lineComment를 단독으로 둘 수 있고, 결함 코멘트와는 **별도**로 분리해도 된다. 스타일만 바꾸는 수준·근거 없는 '더 빠를 것'은 금지.",
              "",
              "── 검증 의무 (추측 금지) ──",
              ...
              "**GFM 구조(강조):** `#` / `##` / `###`으로 **섹션을 나누고**, 빈 줄로 호흡을 준다. 예시·Before/After·패치는 **적절히 fenced 코드블록**을 쓴다. 식·키워드는 `인라인 코드`.",
              `- fenced는 \`\`\`${fenceLang} 로 시작([제출 코드]와 동일 언어). JSON 문자열 안 줄바꿈은 실제 \\n.`,
              "한 문단이 길어지면 **같은 body 안에서** `<br />`를 넣어 시각적으로 환기해도 된다.",
              ...
              "**각 lineComment:** 결함이면 시뮬 완료·분기 흐름·기대≠실제·미실행 경로 근거 아님·4가지 해당·원인↔반례↔수정·근거·가독성·톤·GFM·`<br />`**. **개선 제안만**이면 위 **복잡도·알고리즘** 절의 점근·비교 근거를 충족했는가. 하나라도 미달이면 **그 코멘트 미출력**.",
            ].join("\n"),
          },
          {
            role: "user",
