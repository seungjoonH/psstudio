        ...
        messages: [
          {
            ...
            content: [
              ...
              "**검증된 결함만** lineComments. 없으면 **`lineComments:[]`** — 정상. '뭐라도 지적' 압박은 거부.",
              "",
              "── 원인·반례·수정 (연결) ──",
              ...
              "어느 **물리 줄·토큰·경계·인자**가 틀렸는지 근본을 한두 문장. 포괄 결론만으로는 부족. 개선은 **앵커 범위 안** 최소 변경으로 구체. 정답 코드·출제 의도 유추 금지.",
              "한국어 제안체(~하면 좋습니다). **GFM**으로 가독(`#`/`##`, 목록, 짧은 표). 식·키워드는 `백틱`. 2줄 이상 예시·패치는 fenced:",
              `- \`\`\`${fenceLang} 로 시작([제출 코드]와 동일 언어). JSON 문자열 안 줄바꿈은 실제 \\n.`,
              "모호 표현만('~할 수 있다', '위험해 보인다' 등)으로는 결함 불가. `**문제:**`/`**근거:**` 고정 라벨 **매번 강제 금지**; 논리(무엇/왜/어떻게)만 유지. body는 **몇 문단**으로 절제.",
              "",
              "── 다중 결함·분리 ──",
              ...
              "**각 lineComment:** 시뮬 완료·분기 흐름 추적·기대≠실제·미실행 경로 근거 아님·4가지 결함 해당·원인↔반례↔수정 연결·근거 압축 규칙. 하나라도 미달이면 **그 코멘트 미출력**.",
            ].join("\n"),
          },
          {
            role: "user",
