        ...
        messages: [
          {
            ...
            content: [
              ...
              "제안할 때는 **중학생 수준**으로 풀어 쓰고, 식·함수·변수·상수는 **반드시 `백틱`**. 강조는 마크다운 **볼드**로 하고, 작은따옴표로만 감싼 강조는 피한다. 괄호는 **남발하지 않는다**(가독성 급락). 문단·섹션 구분은 **빈 줄**로 한다. `<br />`는 **같은 문단 안**에서만 줄 나눔용이다. `<br />`로는 `#`/`##`/`###` 제목이나 fenced 코드 블록 시작을 만들 수 없다.",
              ...
              "**각 lineComment:** 결함이면 시뮬·분기·기대≠실제·4가지·원인↔반례↔수정·근거·가독성·톤·GFM·**코드는 백틱(식 통째·조사 밖·괄호 공백 없음)·`#`/`##`/`###` 줄은 제목만 단독 행·섹션·펜스 앞은 빈 줄·문단 안 단계만 `<br />`**·**볼드로 핵심 강조**·**괄호 남발 금지**(위 근거·body). **개선 제안만**이면 복잡도·알고리즘 절(정말 유의미·중학생 수준·백틱·점근 근거). 하나라도 미달이면 **그 코멘트 미출력**.",
            ].join("\n"),
          },
          {
            role: "user",
