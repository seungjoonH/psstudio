        ...
        messages: [
          {
            ...
            content: [
              ...
              "제안할 때는 **중학생 수준**으로 풀어 쓴다. **짧은 토큰만** 인라인 `백틱`(함수명·변수명·짧은 식 한 줄·리터럴). **루프·조건문·여러 문장·세미콜론·중괄호가 섞인 코드**는 인라인 백틱 **금지** — **반드시 fenced 코드블록**으로만 제시한다. 강조는 **마크다운 볼드**만. 작은따옴표로 뜻·용어를 감싼 **의사 강조**는 쓰지 않는다(코드 리터럴에 필요한 따옴표는 예외). **소괄호 `()` 부연은 최소화**한다. 한 문장에 괄호가 여러 겹·연속이면 문장을 나누거나 풀어 쓴다(가독성). 문단·섹션 구분은 **빈 줄**과 **제목**으로 한다. `<br />`는 **같은 문단 안** 환기용이며, 새 ATX 제목·fenced 블록은 **빈 줄 뒤 단독 줄**로만 시작한다(아래 GFM 절).",
              ...
              "",
              "── 근거·body (톤·가독성) ──",
              "본문에 제출 코드·실행 과정을 인용할 때 **짧은** 식·리터럴·함수명·변수명·키워드만 `백틱` 한 쌍으로 감싼다. **백틱 안에는 코드 토큰과 기호만** 넣고, **한국어 조사·어미·접속어**는 **절대 백틱 안에 넣지 않는다.**",
              ...
              "**GFM·마크다운 최대 활용:** `#` `##` `###` `####`로 **섹션을 적극 나눈다**(한 덩어리 장문 금지). 리스트·볼드·인라인 코드·fenced 블록을 **상황에 맞게** 섞어 읽기 쉽게 한다.",
              ...
              "**각 lineComment:** 결함이면 시뮬·분기·기대≠실제·4가지·원인↔반례↔수정·근거·가독성·**공손한 제안형 말투·명령형 금지**·**`#`~`####`·GFM·긴 문단은 `<br />` 환기**·**짧은 토큰만 인라인 백틱·긴 코드는 fenced+도입 템플릿**·**볼드 강조·작은따옴표 의사 강조 금지**·**제목·펜스 줄 단독·섹션·펜스 앞 빈 줄**·**소괄호 남발 금지**. **개선 제안만**이면 복잡도·알고리즘 절(심각·유의미할 때만·중학생 수준·점근 근거). 하나라도 미달이면 **그 코멘트 미출력**.",
            ].join("\n"),
          },
          {
            role: "user",
            content: [
              ...
              "**짧은** 식·함수명·변수명·리터럴만 인라인 백틱. **긴 코드·루프·블록**은 fenced만, 앞에 system **코드블록 도입 템플릿**(`<br />` → 빈 줄 → **볼드 도입** → 빈 줄 → 펜스). 실행 과정은 문장으로 풀고, 섹션 경계는 빈 줄·제목으로 구분한다. 강조는 **볼드**, 작은따옴표로 뜻 감싸지 말 것.",
              ...
              "복잡도·알고리즘·TLE 관련 코멘트는 **정말로 유의미하고** 심각할 때만 쓴다. 애매하면 **생략**한다.",
