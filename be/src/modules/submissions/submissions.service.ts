        ...
        messages: [
          {
            ...
            content: [
              ...
              "lineComments가 비어 있으면 summary는 **긍정·관찰만**; '문제·개선 필요·주의·여지' 등 결함 암시 단어 금지.",
              "매 실행 summary 1개.",
              "",
              "── 결함으로 인정 (4가지만) ──",
              ...
              "제안할 때는 **중학생 수준**으로 풀어 쓰고, 식·함수·변수·상수는 **반드시 `백틱`**. 강조는 마크다운 **볼드**로 하고, 작은따옴표로만 감싼 강조는 피한다. **소괄호 `()` 부연은 최소화**한다. 한 문장에 괄호가 여러 겹·연속이면 문장을 나누거나 풀어 쓴다(가독성). 문단·섹션 구분은 **빈 줄**로 한다. `<br />`는 **같은 문단 안**에서만 줄 나눔용이다. `<br />`로는 `#`/`##`/`###` 제목이나 fenced 코드 블록 시작을 만들 수 없다.",
              ...
              "실행 과정은 **한 줄에 조건을 몰아 쉼표로만 나열**하지 말고, **문장을 끊어** 풀어 쓴다. 단계·문단이 바뀌면 **먼저 빈 줄**로 나누고, **한 문단 안**에서만 줄을 바꿀 때 `<br />`를 쓴다.",
              ...
              "**GFM 구조:** `#`/`##`/`###` 제목 줄과 fenced 블록의 시작·끝(백틱 세 개 이상) 줄은 **각각 단독 한 줄**이어야 한다(앞 공백은 최대 3칸). 제목·펜스 **앞**에는 **빈 줄**로 이전 블록과 끊는다. 설명 문장과 같은 줄에 펜스를 붙이면 코드 블록이 깨진다.",
              "**`<br />`와 빈 줄:** 제품은 마크다운→HTML로 렌더링한다. `<br />`는 **문단 내부** 줄바꿈일 뿐이며, 새 제목·새 fenced 블록·새 문단의 시작이 되지 않는다. `###`·펜스·긴 섹션 경계 앞에는 `<br />` 연속을 쓰지 말고 **반드시 빈 줄**을 넣는다. 예시·패치는 **fenced 코드블록**으로만 제시한다.",
              ...
              "**각 lineComment:** 결함이면 시뮬·분기·기대≠실제·4가지·원인↔반례↔수정·근거·가독성·**공손한 제안형 말투·명령형 금지**·GFM·**코드는 백틱(식 통째·조사 밖·괄호 공백 없음)·`#`/`##`/`###` 줄은 제목만 단독 행·섹션·펜스 앞은 빈 줄·문단 안 단계만 `<br />`**·**볼드로 핵심 강조**·**소괄호 남발 금지**(위 근거·body). **개선 제안만**이면 복잡도·알고리즘 절(정말 유의미·중학생 수준·백틱·점근 근거). 하나라도 미달이면 **그 코멘트 미출력**.",
            ].join("\n"),
          },
          {
            role: "user",
            content: [
              ...
              "body는 **빈 줄**로 섹션을 나누고, `#`/`##`/`###`·fenced 코드블록으로 구조를 잡으며 **가독성**을 챙긴다. `<br />`는 문단 **안**에서만 쓴다. 제목 줄에는 제목만, 같은 줄에 본문·백틱·펜스를 붙이지 않는다.",
              "본문의 코드·식·함수명·변수명·리터럴은 모두 백틱으로 감싼다. 실행 과정은 문장으로 풀고, 문단이 바뀌면 빈 줄로 구분한다. 같은 문단 안에서만 단계 나눔에 `<br />`를 쓴다.",
