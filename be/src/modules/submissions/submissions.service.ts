        ...
        messages: [
          {
            ...
            content: [
              ...
              "**한 lineComment에 여러 독립 결함을 장문으로 묶어 총정리하지 않는다.** \"문제 요약\"처럼 전체를 한 덩어리 서술하는 형식은 금지한다. 각 결함은 **짧은 별도 코멘트**로 나눈다.",
              "",
              "── lineComment 작성 조건 ──",
              ...
              "**담백함·절제:** 각 body는 **몇 문단 안**에서 끝낸다. 통합 서두·장황한 \"수정 검증\" 전개·같은 말 반복은 피한다. 반례·기대 vs 실제는 **한 블록에 넣기 좋은 최소 분량**만.",
              ...
              "**응답 전체:** 첫 결함을 찾은 뒤에도 코드 전체를 끝까지 검토해, 독립된 추가 결함을 놓치지 않았는가? (위 \"다중 결함 탐색 규칙\") **독립 원인은 별도 lineComment로 나누었는가? 한 코멘트에 여러 버그를 장문으로 합치지 않았는가?**",
              ...
              "- **범위:** 결함의 **직접 원인이 되는 줄(또는 한 식)**을 우선 앵커의 중심으로 둔다. 읽는 사람이 맥락을 이해하려면 **그 줄 바로 위·아래 필요한 줄만** 포함해 start/end를 조정한다. **함수 전체·호출부+정의부를 한 블록에 길게 묶지 않는다.** 호출과 정의가 둘 다 틀리면 **lineComment를 둘로 나누고** 각각 최소 구간을 잡는다.",
              ...
              "- lineComments 최대 10개. **동일 결함·동일 최소 수정**으로 해결되는 내용은 한 lineComment로 묶고, 여러 코멘트로 쪼개지 않는다. **서로 다른 줄·서로 다른 직접 원인**이면 반드시 **별도 lineComment**로 나눈다.",
          ...
          },
          {
            ...
            content: [
              ...
              "각 코멘트는 **그 앵커 구간만** 짧고 담백하게. 여러 버그를 한 코멘트에 총정리하지 말고, 오류 줄을 먼저 정한 뒤 **코멘트·앵커를 결함마다** 나눠. 맥락은 앵커를 위아래로 **최소한만** 넓혀.",
