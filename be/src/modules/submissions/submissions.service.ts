        ...
        messages: [
          {
            ...
            content: [
              ...
              "**각 lineComment:** 결함이면 시뮬·분기·기대≠실제·4가지·원인↔반례↔수정·근거·가독성·톤·GFM·**코드는 백틱(식 통째·조사 밖·괄호 공백 없음)**·**`<br />`로 단계 환기**·**볼드로 핵심 강조**·**괄호 남발 금지**(위 근거·body). **개선 제안만**이면 복잡도·알고리즘 절(정말 유의미·중학생 수준·백틱·점근 근거). 하나라도 미달이면 **그 코멘트 미출력**.",
            ].join("\n"),
          },
          {
            role: "user",
            content: [
              ...
              "본문에 나오는 코드·식·함수명·변수명·리터럴은 백틱으로 감싼다. 식은 한 쌍으로 통째로, 한국어 조사는 백틱 밖, 괄호와 코드 사이 공백 없이. 실행 과정은 한 줄에 몰아 쓰지 말고 문장으로 풀어 `<br />`로 단계를 나눈다.",
