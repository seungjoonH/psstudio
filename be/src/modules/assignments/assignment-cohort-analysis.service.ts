...
function cohortLocaleOverride(locale: CohortReportLocale): string[] {
  if (locale === "en") {
    return [
      ...
      "- **One `roleId` = one logical responsibility.** Different purposes (time normalization vs weekday/weekend skip vs lateness threshold vs accumulation) require **different `roleId`s** (different highlight colors). Never label a span \"weekend check\" while anchoring helpers, outer loops, or unrelated comparisons inside it.",
