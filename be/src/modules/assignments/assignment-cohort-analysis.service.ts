...
function cohortLocaleInstruction(locale: CohortReportLocale): string {
  ...
  return [
    ...
    "한국어 본문에서 코드 펜스를 소개할 때 문장을 콜론(`:`)으로 끝내지 않는다. 특히 `다음과 같습니다:`, `다음과 같다:` 금지. 앞 문장은 마침표로 닫고, 펜스·목록은 그 다음 줄에 둔다. 시간 표기·코드 안의 `:`·마크다운 링크 문법은 그대로 둔다.",
...
function cohortMarkdownStructureAndCodeRules(locale: CohortReportLocale): string[] {
  ...
  return [
    ...
    "- **콜론 금지(한국어 본문):** 인용·코드 블록을 소개할 때 `다음과 같습니다:`처럼 문장을 콜론으로 끝내지 않는다. `다음과 같습니다.`로 끝내고 코드 펜스는 **그 다음 줄**부터 시작한다.",
  ...
  private async runPipeline(analysisId: string): Promise<void> {
    ...
    try {
      ...
      const systemPrompt = [
        ...
        "[reportMarkdown]",
        "- 마크다운으로 작성한다.",
        "- 긴 설명은 **문단 단위**로 끊는다. **문단 사이**에만 빈 줄 한 줄 또는 `<br /><br />` 한 번(문장마다 `<br />` 금지). 예시 같은 호흡. 첫 문단 끝 → 빈 줄 → 둘째 문단 → 빈 줄 → `코드의 일부는 다음과 같습니다.`(마침표) → 빈 줄 → 코드 펜스.",
        ...
        "- **한국어 서술에서 `다음과 같습니다:` 등 설명 뒤 콜론으로 펜스를 소개하지 않는다.** 마침표로 끝내고 펜스는 다음 줄에 둔다.",
