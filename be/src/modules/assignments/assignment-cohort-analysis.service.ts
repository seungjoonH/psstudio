...
function cohortLocaleOverride(locale: CohortReportLocale): string[] {
  ...
  return [
    ...
    "- **`reportMarkdown`은 독자용 글이다.** `roleId`·`startAnchorText`·`endAnchorText`·`regions` 같은 **JSON 필드명·내부 식별자**를 제목·설명 문장에 **절대 쓰지 않는다.** 내부 id를 **인라인 백틱**으로 감싸 넣지도 않는다(코드 발췌 ``` 펜스는 그대로 허용). `##` 제목은 비교 주제만 자연어(한글)로 쓴다. 「각 `##`의 백틱은 …」「이 절은 어떤 구역을 가리킨다」처럼 **마크다운 구조·색 하이라이트·데이터 모델을 설명하는 메타 문장**도 금지다. 화면 구역 연동은 JSON만으로 처리된다.",
