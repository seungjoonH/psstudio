...
function cohortLocaleInstruction(locale: CohortReportLocale): string {
  ...
  return [
    ...
    "`[[SUBMISSION:id]]` 뒤에서만 줄을 바꿔 `: ○○ 제출`·조사 `의`만 다음 줄에 두지 않는다. 칩과 뒤의 서술은 같은 줄에서 한 문장으로 이어 쓴다.",
...
function cohortMarkdownStructureAndCodeRules(locale: CohortReportLocale): string[] {
  ...
  return [
    ...
    "- **문단 간 호흡:** 문단마다 **빈 줄**을 두는 것을 기본으로 한다. 필요하면 문단과 문단 사이에 `<br /><br />`를 **한 번만** 넣어 간격을 줄 수 있다. **문장마다 `<br />`를 넣으면 안 된다.**",
  ...
  private async runPipeline(analysisId: string): Promise<void> {
    ...
    try {
      ...
      const systemPrompt = [
        ...
        "[reportMarkdown]",
        "- 마크다운으로 작성한다.",
        "- 긴 설명은 **문단 단위**로 끊는다. 문단 사이는 빈 줄 또는 `<br /><br />` **정도만**(문장마다 `<br />` 금지).",
        ...
        "- **`[[SUBMISSION:id]]` 직후에만 줄을 바꿔 `: ○○ 제출` 또는 다음 줄에 조사 `의`만 두지 않는다.** 칩과 뒤따르는 서술은 **한 문장이 되도록 같은 줄에서** 이어 쓴다(예: `[[SUBMISSION:uuid]]의 time_adjust는 …`, `[[SUBMISSION:uuid]]에서는 …`).",
