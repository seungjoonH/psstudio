...
function cohortLocaleOverride(locale: CohortReportLocale): string[] {
  if (locale === "en") {
    return [
      ...
      "- **`roleLabel` length is hard-capped: 1–3 words, ≤ 18 visible characters.** No articles only — pick concrete content words. If the role needs more nuance, drop it into prose, not the label. Examples — Good: `time normalization`, `weekend skip branch`, `late check loop`, `total accumulation`. Bad: `the part that computes weekday using mod and skips weekends`, `outer loop with weekday check and lateness comparison`, `aggregating results and returning final answer`. The label sits in a small chip overlaying code; longer labels obscure the source.",
      ...
      "- **Never chain multiple statements inside one inline backtick.** Bad: `` `m += 10; h += m / 60; m %= 60;` ``. Good: \"…by `m += 10`, `h += m / 60`, then `m %= 60`.\" Split each statement into its own backtick joined with prose; if you need to show several lines together, use a fenced block.",
      "- **Never put a `[[SUBMISSION:<uuid>]]` placeholder inside any backtick** — not inside an inline pair `` ` ` ``, not inside a fenced ``` block. Placeholders must live in plain prose only.",
      "  - Bad: ``the two solutions (`[[SUBMISSION:a]], [[SUBMISSION:b]]`) both use…`` (placeholders trapped inside backticks break the chip rendering)",
      "  - Bad: ``[[SUBMISSION:a]] runs `for (let i = 0; i < n; i++) { … [[SUBMISSION:b]] … }` so…`` (a second placeholder inside backticks)",
      "  - Good: `the two solutions [[SUBMISSION:a]] and [[SUBMISSION:b]] both use the `HHMM` format so…`",
      "- **Inline backticks must open and close on the same physical line.** No newline, `<br />`, or `[[SUBMISSION:…]]` placeholder may sit between an opening `` ` `` and its closing `` ` ``. A single unbalanced backtick paints the rest of the document as code.",
      "- **Never leave a single `` ` `` (or two) on a line by itself.** Code fences MUST always be three backticks: ` ```language` to open, ` ``` ` to close, each on its own dedicated line.",
      "- **Do not wrap `[[SUBMISSION:…]]` chips inside parens-with-backticks** like ``(two solutions: `[[a]], [[b]]`)``. If you want them in parens, write `(two solutions [[SUBMISSION:a]], [[SUBMISSION:b]])` with no backticks around the chips.",
      "- **Self-check before emitting JSON:** (1) the inline `` ` `` count in `reportMarkdown` is even, (2) no backtick scope contains a `[[SUBMISSION:…]]` placeholder, (3) every inline backtick opens and closes on the same line, (4) no stray solo `` ` `` line, (5) every fence opens and closes with three `` ``` `` markers. Fail any check → rewrite `reportMarkdown` before emitting.",
      "- **Avoid the words \"excerpt\", \"snippet excerpt\", or stiff equivalents** when introducing code. Just describe what the submission does (\"in `[[SUBMISSION:…]]`, the conversion is handled by …\") and let the fenced block follow naturally.",
      "- **Closing section title** should be friendly natural prose (e.g. `## Wrap-up`, `## Putting it together`). Avoid stiff labels like `## Closing remarks` / `## Epilogue` / `## 맺음말`.",
      ...
      "- **Vary sentence endings across submission blocks.** Do not paste the same closing pattern for every `### [[SUBMISSION:…]]` (e.g. repeating \"…is the part that…\" then \"below shows…\" for each submission). If two adjacent paragraphs would end the same way, rewrite one with a different rhythm (question, contrast, short aside, different verb). Before emitting JSON, skim `reportMarkdown` and fix any phrase that appears **3+ times** as a near-identical tail.",
  ...
  return [
    ...
    "- **`roleLabel`은 짧고 단단한 명사구로만 — 띄어쓰기 기준 1~3 어절·총 12자 이내.** 어절은 띄어쓰기 한 칸으로 끊는 단위(예: 「시간 환산」=2어절, 「주말 분기 처리」=3어절, 「지각 누적 갱신」=3어절). **「~하는 부분」, 「~을 만드는 ~」, 「~에서 ~을 ~하는」 같은 서술형 꼬리·관형절·조사로 길게 늘인 라벨은 금지.** 라벨에 다 못 담는 뉘앙스는 본문 서술로 풀고, 라벨은 화면에서 음영 위에 떠 있는 작은 칩이라 길어지면 코드를 가린다.",
    "  - 좋은 예: 「시간 환산」, 「주말 분기 처리」, 「지각 누적 갱신」, 「입력 정규화」, 「반환·후처리」, 「분 단위 비교」.",
    "  - 잘못된 예(이번에 실제로 길게 뽑힌 라벨): 「출근 희망 시각에 10분을 더해 비교 기준을 만드는 부분」(15어절), 「평가 결과를 집계하고 최종 반환하는 부분」(7어절), 「요일 판정 후 평일만 검사에서 지각을 찾는 반복문」(7어절). 모두 **3어절·12자** 이하로 줄여야 한다(예: 「희망 시각 보정」, 「결과 집계·반환」, 「평일 지각 탐색」).",
    "  - **JSON 출력 직전, 모든 region의 `roleLabel`을 **하나하나** 어절 수와 글자 수를 세어 본다.** 4어절 이상이거나 12자를 넘으면 그 라벨을 **반드시 다시 작성**하고 출력한다.",
    ...
    "- **한 인라인 백틱 안에 여러 statement를 묶지 마라.** `;`(세미콜론)으로 두 줄 이상의 코드를 한 백틱에 이어 붙이지 않는다. 잘못된 예: `` `m += 10; h += m / 60; m %= 60;` ``. 올바른 예: ``과 같이 `m += 10`, `h += m / 60`, `m %= 60` 으로 풀어 씁니다.`` 식으로 **statement마다 백틱을 따로** 두고 사이를 한국어 조사·접속어로 잇는다. 한 번에 두 줄 이상을 보여 주고 싶으면 ``` 펜스로.",
    "- **`[[SUBMISSION:<uuid>]]` 태그는 절대 어떤 백틱 안에도 들어가지 않는다.** 인라인 백틱 한 쌍 안에도, ``` 펜스 코드 블록 안에도 태그를 넣지 마라. 태그는 항상 **평문 한국어 사이**에만 둔다.",
    "  - 잘못된 예: ``요약하면, 두 제출(`[[SUBMISSION:a]], [[SUBMISSION:b]]`)은 …`` (태그를 백틱+괄호로 묶어 chip 자리가 망가진다)",
    "  - 잘못된 예: ``[[SUBMISSION:a]]는 `for (let i = 0; i < n; i++) { … [[SUBMISSION:b]] … }` 식으로 …`` (백틱 안에 다른 태그가 들어가 chip 출력이 깨진다)",
    "  - 올바른 예: `요약하면 [[SUBMISSION:a]]와 [[SUBMISSION:b]]는 `HHMM` 포맷을 …`",
    "- **인라인 백틱 한 쌍은 반드시 같은 한 줄 안에서 열고 닫는다.** 여는 `` ` `` 와 닫는 `` ` `` 사이에는 줄바꿈·`<br />`·`[[SUBMISSION:…]]` 태그가 한 글자도 들어가면 안 된다. 줄바꿈을 가로지르는 인라인 백틱이 단 하나라도 있으면 화면 전체가 코드 음영으로 덮인다.",
    "- **단독 줄에 `` ` `` 한 개만 두지 마라.** 코드 펜스를 시작·종료할 때는 **반드시 `` ``` `` 세 개**를 쓴다. ` ```언어` 한 줄로 펜스를 열고 ` ``` ` 한 줄로 닫는다. ` ` 백틱 한 개·두 개로 펜스를 흉내 낸 줄이 절대 출력에 남으면 안 된다.",
    "- **인라인 백틱 안에 괄호로 chip을 묶는 패턴 금지.** 「두 제출(`태그, 태그`)은」, 「세 제출(`태그·태그·태그`) 모두」 같은 출력 금지. 괄호 안에 태그를 나열하고 싶으면 **백틱 없이** `(두 제출 [[SUBMISSION:a]], [[SUBMISSION:b]])` 처럼 쓴다.",
    "- **출력 직전 `reportMarkdown` 통독 점검(필수):** ① 인라인 백틱 `` ` `` 총 개수가 짝수인가, ② 어떤 인라인 백틱·`` ``` `` 펜스 안에도 `[[SUBMISSION:…]]` 태그가 들어가 있지 않은가, ③ 인라인 백틱이 줄바꿈을 가로지르지 않는가(여는 백틱과 닫는 백틱이 같은 줄에 있는가), ④ 단독 줄로 떠 있는 `` ` `` 한 개·두 개가 없는가, ⑤ 모든 펜스가 `` ``` `` 세 개로 열고 닫혀 짝이 맞는가. 하나라도 실패하면 JSON 출력을 멈추고 `reportMarkdown`을 다시 작성한다.",
    "- **`발췌`라는 단어는 reportMarkdown에 쓰지 않는다.** 「~의 발췌」「~을 발췌하면」「발췌 코드」 같은 표현 대신, **`[[SUBMISSION:…]]`은 어떻게 처리하는지** / **어떤 식으로 구현되어 있는지** / **이 구간을 보면** 처럼 **자연스럽게 풀어** 쓴다. 코드를 보여 줄 때도 라벨 없이 짧은 볼드 도입(예: `**예:**`) 또는 평문 한 줄로 자연스럽게 잇는다.",
    "- **인라인 백틱에 JSON 내부 필드명·식별자를 넣지 않는다.** `roleId`·`startAnchorText`·`endAnchorText`·`regions` 같은 데이터 모델 단어는 **본문·제목·백틱 어디에도** 노출하지 않는다(코드 ``` 펜스 자체는 허용). `##` 제목은 비교 주제만 자연어 한 줄로 쓴다. 「각 `##`의 백틱은 …」「이 절은 어떤 구역을 가리킨다」처럼 **마크다운 구조·색 하이라이트·데이터 모델을 설명하는 메타 문장도 금지.** 화면 구역 연동은 JSON만으로 처리된다.",
    ...
    "- **문장 종결·도입 패턴을 제출마다 복붙하지 마라(특히 `### [[SUBMISSION:…]]` 나열).** 「~**부분입니다.**」「~**부분입니다.**」처럼 같은 어미를 연속으로 쓰지 말고, 「아래 … **보여줍니다.**」「다음 … **보여줍니다.**」처럼 **같은 문장 꼬리**(끝 4~8자가 거의 같은 패턴)를 **인접한 두 블록**에서 반복하지 마라. **제출마다 다른 종결·도입**을 번갈아 쓴다(예: ~구간에 모여 있습니다, ~에서 확인할 수 있습니다, ~로 이어집니다, 짧은 질문으로 넘기기, 앞 제출과의 대비 한 문장 등). JSON 출력 직전 `reportMarkdown` 전체를 훑어 **같은 어미가 3회 이상** 나오면 문장을 고친다.",
...
function buildCohortSystemPrompt(locale: CohortReportLocale): string {
  ...
  return [
    ...
    "",
    "[ABSOLUTE — 다른 모든 룰보다 우선]",
    "- **`[[SUBMISSION:<uuid>]]` 태그는 백틱과 절대 같이 쓰지 마라.** 인라인 백틱 한 쌍 `` `…` `` 안에도, ``` 펜스 코드 블록 안에도, **태그를 감싸는 어떤 백틱도 출력하지 마라.** 태그 옆에 백틱 한 짝만 잘못 두어도 본문 절반이 코드 음영으로 덮이고 칩이 사라진다.",
    "- **특히 자주 어기는 잘못된 패턴 — 출력 금지:**",
    "  - ``…택해 산술이 명확합니다(`[[SUBMISSION:a]]`)을 사용합니다.`` (태그 한 개를 백틱으로 감쌈)",
    "  - ``직접 보정하는 방식(`[[SUBMISSION:a]], [[SUBMISSION:b]]`)을 사용합니다.`` (태그 여러 개를 한 백틱으로 묶음)",
    "  - ``두 제출 (`[[SUBMISSION:a]]`, `[[SUBMISSION:b]]`)은…`` (괄호 안 각 태그를 백틱으로 따로 감쌈)",
    "- **올바른 형태(아래 둘 중 하나만 허용):**",
    "  - 괄호 없이 평문에 그대로: `…택해 산술이 명확합니다 [[SUBMISSION:a]].`",
    "  - 괄호 안에 평문으로: `…직접 보정하는 방식([[SUBMISSION:a]], [[SUBMISSION:b]])을 사용합니다.`",
    "- **검증 절차(JSON 출력 직전 필수):** `reportMarkdown`을 한 번 통독하면서, `[[SUBMISSION:` 가 등장하는 모든 위치 직전·직후 한 글자가 `` ` `` 인지 확인한다. 단 한 곳이라도 백틱이 붙어 있으면 그 문장 전체를 다시 작성한 뒤에만 JSON을 낸다.",
    "",
    "[OUTPUT CONTRACT]",
    ...
    "  - **`roleLabel` 길이 하드캡:** 한국어는 **띄어쓰기 기준 1~3 어절·총 12자 이내**, 영문은 **1~3 word·총 18 visible char 이내**. 「~하는 부분」「~을 만드는 ~」「~에서 ~을 찾는 반복문」 같은 서술형·관형절 라벨은 모두 금지(이런 라벨은 화면 칩이 코드 위로 길게 깔린다). 다 못 담는 의미는 본문 서술로 풀고, 라벨은 짧은 명사구로만 짓는다. (예: 좋음 — 「시간 환산」, 「주말 분기 처리」, 「지각 누적 갱신」, 「희망 시각 보정」, 「평일 지각 탐색」, 「결과 집계·반환」 / 잘못 — 「출근 희망 시각에 10분을 더해 비교 기준을 만드는 부분」, 「평가 결과를 집계하고 최종 반환하는 부분」, 「요일 판정 후 평일만 검사에서 지각을 찾는 반복문」.)",
    ...
    "- 코드 비교는 fenced 코드 블록(```)을 사용하며 전체 파일 붙여넣기 금지.",
    "- 각 `##` 축 섹션마다 최소 1개 이상의 펜스 코드 블록(``` 언어태그 + region 앵커 안 **원문** 일부)이 있어야 한다. 서술만 두고 스니펫 없이 해당 축을 끝내지 않는다.",
    ...
    "- 스니펫은 반드시 설명과 같은 축/같은 제출의 region 앵커 구간 안에서만 가져온다. 설명과 무관한 코드 인용 금지.",
    ...
    "- **마지막 마무리 절 제목은 「마무리」 또는 자연스러운 한국어 한 줄**(예: `## 마무리`, `## 정리하며`, `## 종합 비교`). 「맺음말」·「에필로그」 같은 딱딱한 단어는 쓰지 않는다.",
    ...
    "- **한 인라인 백틱에 여러 statement를 묶지 마라.** 세미콜론으로 두 줄 이상의 코드를 한 백틱에 이어 붙이지 않는다. 잘못된 예: `` `m += 10; h += m / 60; m %= 60;` ``. 올바른 예: ``과 같이 `m += 10`, `h += m / 60`, `m %= 60` 으로 풀어 씁니다.`` 처럼 **statement마다 백틱을 따로** 두고 사이를 한국어 조사·접속어로 잇는다. 두 줄 이상을 한 번에 보여 주고 싶으면 ``` 펜스로.",
    "- **[BACKTICK HYGIENE — 깨짐 방지 최우선]** 백틱은 깨지기 매우 쉬운 구조라 아래 다섯 가지를 모두 지킨다.",
    "  1) **`[[SUBMISSION:<uuid>]]` 태그는 모든 백틱 바깥에만.** 인라인 백틱 한 쌍 사이에도, ``` 펜스 코드 블록 안에도 태그를 절대 넣지 마라. 태그 자리에 화면에서는 작성자 칩이 들어가야 하므로, 백틱 안에 갇히면 chip이 사라지고 코드 음영이 본문을 덮는다.",
    "  2) **인라인 백틱 한 쌍은 같은 한 줄에서 열고 닫는다.** 여는 `` ` `` 와 닫는 `` ` `` 사이에 줄바꿈·`<br />`·`[[SUBMISSION:…]]`·다른 백틱이 들어가면 안 된다. 줄을 가로지르는 인라인 백틱이 단 하나만 있어도 본문 절반이 코드 음영으로 덮인다.",
    "  3) **펜스는 항상 `` ``` `` 세 개**다. ` ```언어` 로 한 줄 단독 시작, ` ``` ` 로 한 줄 단독 종료. 백틱 한 개·두 개만 단독 줄로 떠 있으면 안 된다. 「``` 한 개를 깜빡 빼먹은 줄」이 없어야 한다.",
    "  4) **백틱과 괄호로 chip을 묶는 패턴 금지.** 「두 제출(``[[SUBMISSION:a]], [[SUBMISSION:b]]``)은」, 「세 제출(``[[a]]·[[b]]·[[c]]``) 모두」 같은 출력은 모두 깨진다. 괄호 안에 태그를 나열하고 싶으면 **백틱 없이** `(두 제출 [[SUBMISSION:a]], [[SUBMISSION:b]])` 처럼 쓴다.",
    "  5) **출력 직전 `reportMarkdown` 통독 점검:** ① 인라인 `` ` `` 총 개수가 짝수인가, ② 어떤 백틱·펜스 안에도 `[[SUBMISSION:…]]` 가 들어가 있지 않은가, ③ 인라인 백틱이 줄바꿈을 가로지르지 않는가, ④ 단독 줄로 떠 있는 `` ` `` 한 개·두 개가 없는가, ⑤ 모든 펜스가 `` ``` `` 세 개로 짝을 이루는가. 하나라도 실패하면 JSON 출력을 멈추고 `reportMarkdown`을 다시 작성한다.",
    "  - 잘못된 예(실제 깨짐 케이스): ``요약하면, 두 제출(`[[SUBMISSION:a]], [[SUBMISSION:b]]`)은 `HHMM` 포맷을…`` — 첫 백틱이 chip 두 개를 감싸 버려 본문 절반이 코드 음영으로 덮인다.",
    "  - 올바른 예: `요약하면 [[SUBMISSION:a]]와 [[SUBMISSION:b]]는 `HHMM` 포맷을 직접 보정하는 방식입니다.` — 태그는 평문, `HHMM` 만 인라인 백틱.",
    "- **`발췌`라는 단어를 본문에 쓰지 마라.** 「~의 발췌」「발췌하면」「발췌 코드」 같은 표현 대신 **「`[[SUBMISSION:…]]`에서는 ~을 어떤 식으로 구현했는지」** / **「이 구간을 보면」** / **「~로 처리되어 있습니다」** 처럼 자연스럽게 풀어 쓴다. 코드 펜스도 라벨 없이 짧은 볼드 도입(예: `**예:**`) 또는 평문 한 줄로 잇는다.",
    ...
    "- **문장 끝·펜스 앞 도입문이 제출마다 똑같이 반복되지 않게 한다.** 「~**부분입니다.**」「아래 … **보여줍니다.**」처럼 **고정 틀**을 `### [[SUBMISSION:…]]` 블록마다 복붙하지 마라. 인접한 두 제출 블록에서 **같은 어미로 끝나는 문장**이 나오면 하나를 다른 말로 고친다. JSON을 내기 전에 reportMarkdown을 통독해 **같은 꼬리 패턴이 3회 이상**이면 반드시 고친다.",
    ...
    "- [ ] **모든 `roleLabel`이 한국어 1~3 어절·12자 이내(영문 1~3 word·18 char 이내)인가**(「~하는 부분」, 「~을 ~하는 ~」 같은 서술형·관형절 라벨이 단 한 개도 없는가)",
    ...
    "- [ ] 「~부분입니다.」「~보여줍니다.」처럼 **같은 문장 꼬리**가 인접한 `### [[SUBMISSION:…]]` 블록에서 연속으로 나오지 않았는가(같은 어미가 reportMarkdown 전체에서 **3회 이상** 반복되지 않았는가)",
    ...
    "- [ ] 한 인라인 백틱 안에 세미콜론으로 두 개 이상의 statement를 묶어 두지 않았는가(예: `` `m += 10; h += m / 60` `` 같은 출력 없는가)",
    "- [ ] **`reportMarkdown` 내 인라인 백틱 `` ` `` 의 총 개수가 짝수인가**(불균형이 한 군데라도 있으면 본문이 코드 음영으로 덮인다)",
    "- [ ] **어떤 인라인 백틱·`` ``` `` 펜스 안에도 `[[SUBMISSION:…]]` 태그가 들어가 있지 않은가**(태그는 평문 사이에만)",
    "- [ ] **`[[SUBMISSION:…]]` 태그의 바로 앞·바로 뒤 한 글자가 `` ` `` 인 자리가 한 군데도 없는가**(예: ``(`[[SUBMISSION:a]]`)`` 같은 출력 금지)",
    "- [ ] **인라인 백틱 한 쌍이 같은 한 줄 안에서 열고 닫히는가**(여는 `` ` `` 와 닫는 `` ` `` 사이에 줄바꿈·`<br />`·다른 태그가 없는가)",
    "- [ ] **단독 줄로 떠 있는 `` ` `` 한 개·두 개가 한 줄도 없는가**(펜스를 쓰려면 항상 `` ``` `` 세 개)",
    "- [ ] **모든 펜스가 `` ``` `` 세 개로 시작하고 `` ``` `` 세 개로 닫혀 짝이 맞는가**(닫는 펜스를 빠뜨리지 않았는가)",
    "- [ ] **백틱과 괄호로 chip을 묶는 패턴**(예: ``(두 제출 `[[a]], [[b]]`) 은``)이 한 군데도 없는가",
    "- [ ] 본문에 `발췌`라는 단어가 한 번도 등장하지 않는가(코드 인용 도입은 자연스러운 풀이 문장으로)",
    "- [ ] 마지막 절 제목이 「마무리」류 자연어인가(「맺음말」·「에필로그」 단어가 없는가)",
  ...
  private async runPipeline(analysisId: string): Promise<void> {
    ...
    try {
      ...
      const userPromptBase = [
        ...
        "- **`roleLabel` 길이 하드캡: 한국어 1~3 어절·12자 이내(영문 1~3 word·18 char 이내).** 라벨은 화면 칩이라 길어지면 코드를 가린다. 「~하는 부분」「~을 ~하는 반복문」 같은 서술형은 금지하고 짧은 명사구로만(예: 「희망 시각 보정」, 「평일 지각 탐색」, 「결과 집계·반환」). JSON 출력 직전 모든 `roleLabel`의 어절 수·글자 수를 다시 세고, 4 어절 이상이거나 12자 초과면 그 라벨을 다시 작성한다.",
        ...
        "- reportMarkdown 종결 반복 금지: 「~부분입니다.」「아래 … 보여줍니다.」 같은 **고정 어미**를 제출 블록마다 복붙하지 말고, 제출마다 **다른 종결·도입**을 번갈아 쓴다. 인접한 두 `### [[SUBMISSION:…]]`에서 같은 꼬리 패턴이 나오면 하나를 고친다.",
        ...
        "- reportMarkdown 인라인 백틱 분할: 한 백틱에 세미콜론으로 여러 statement를 묶지 않는다. ``  `m += 10; h += m / 60; m %= 60;`  `` 같은 출력은 ``과 같이 `m += 10`, `h += m / 60`, `m %= 60` 으로 풀어 씁니다.`` 처럼 **statement마다 백틱을 따로** 둔다.",
        "- reportMarkdown 백틱 hygiene(깨짐 방지 최우선): ① **`[[SUBMISSION:…]]` 태그는 모든 백틱 바깥** — 인라인 백틱·`` ``` `` 펜스 안에 절대 넣지 마라. 잘못된 예 ``요약하면 두 제출(`[[a]], [[b]]`)은…``, ``…택해 산술이 명확합니다(`[[a]]`)을 사용합니다.``, ``…방식(`[[a]], [[b]]`)을 사용합니다.`` 같은 출력 금지. 태그를 괄호와 함께 쓰고 싶다면 **괄호 안도 평문**: `([[SUBMISSION:a]])` / `([[SUBMISSION:a]], [[SUBMISSION:b]])`. ② 인라인 백틱 한 쌍은 **같은 한 줄**에서 열고 닫는다(줄바꿈·태그가 끼면 안 됨). ③ 펜스는 항상 `` ``` `` 세 개로만 — 단독 줄에 `` ` `` 한 개·두 개가 떠 있으면 안 된다. ④ 출력 직전 `reportMarkdown` 의 모든 `[[SUBMISSION:` 앞뒤 한 글자를 확인해 백틱이 붙어 있지 않은지·인라인 `` ` `` 총 개수가 **짝수**인지·모든 펜스가 짝을 이루는지 점검 후 JSON을 낸다.",
        "- reportMarkdown 단어 금지: 본문에 `발췌`라는 단어를 쓰지 않는다. 코드 인용을 도입할 때 「~의 발췌」「발췌 코드」 같은 표현 대신 「`[[SUBMISSION:…]]`에서는 ~로 처리되어 있습니다」 식으로 자연스럽게 잇는다.",
        "- reportMarkdown 마지막 절: 마무리 절 제목은 「마무리」 또는 자연스러운 한국어 한 줄(예: `## 마무리`, `## 정리하며`, `## 종합 비교`). 「맺음말」·「에필로그」 같은 단어는 쓰지 않는다.",
