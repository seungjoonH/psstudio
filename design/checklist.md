...
- 체크리스트 반영 완료 여부: yes
- 핵심 결정
  - 운영 배포는 `design/architecture.md` 9장의 GCE `e2-small` 단일 VM 구성을 따른다.
  - Caddy가 `/`는 FE, `/api/*`는 BE로 라우팅하고 Redis는 VM 내부 컨테이너로만 노출한다.
  - GHCR 이미지를 빌드하고 SSH로 VM에서 `docker compose pull && docker compose up -d`를 실행한다.
