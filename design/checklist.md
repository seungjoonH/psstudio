...
> 2026-05-07 추가 합의(그룹·AI·이메일 초대 모델 변경) 요약. 2026-05-09에 그룹 규칙에서 기계 번역·`translationLanguage`·집단 분석 공통 언어 전제를 제거했습니다(`design.md` 5.4.4~5.4.5). 자세한 내용은 §2.12와 최신 `design/*.md`를 따릅니다.
...
레거시 키 `LLM_API_KEY`는 제거하고, OpenRouter는 **`OPEN_ROUTER_API_KEY`**로 통일합니다. 이메일 키(`RESEND_API_KEY`, `EMAIL_FROM_ADDRESS`)는 `be/.env.local`에만 둡니다(worker는 BE 큐를 통해 메일 발송 트리거를 받습니다).

### 2.12 그룹·AI·이메일 초대 모델 합의(2026-05-07 2차, 2026-05-09 번역·집단 공통언어 제거 반영)

다음은 5/7 2차 회의로 새로 합의된 내용입니다. **2026-05-09**에 제출 코드 기계 번역(`SUBMISSION_TRANSLATIONS`, 그룹 규칙 `translationLanguage`)은 **구현 범위에서 제외**했고, 집단 코드 비교는 **제출별 원문 코드 + LLM `regions`만** 저장합니다. 기존 §5.6 피드백 공개 정책 매트릭스, 일회성 초대 링크 만료 모델, AI 분석 결과 별도 패널은 모두 폐기합니다.
...
- 가입 방식 2가지(`코드`, `링크`)만 그룹별 토글로 켜고 끔. 모두 끄면 가입 동결. (가입 신청·이메일 초대는 제거됨.)
- 이메일 초대는 그룹당 1시간 내 50건 발송 제한. 같은 주소 PENDING 토큰이 있으면 기존 토큰을 만료 처리한 뒤 새 토큰 발급.
- 그룹 폼 필드: 그룹명(필수, 20자), 그룹 설명(optional, plain text 500자), 최대 인원수(default 10, 한도 50), 그룹 코드 표시 + 코드 갱신(그룹장만), 가입 방식 4 토글, 그룹 규칙 6항목.
- 그룹 규칙 6항목: `useDeadline`, `defaultDeadlineTime`, `allowLateSubmission`, `useAiFeedback`, `allowEditAfterSubmit`, `assignmentCreatorRoles`. 모두 기본값을 가진다.
...
- ERD 변경 영향: `INVITE_LINKS`, `INVITE_CODES`, `GROUP_FEEDBACK_POLICIES`, `ASSIGNMENT_POLICY_OVERRIDES`, `AI_ANALYSES` 폐지. 신규 또는 변경: `GROUPS`(설명/최대 인원/그룹 코드/가입 토글/그룹 규칙 컬럼 추가), `GROUP_INVITE_LINKS`(영구 토큰 모델), `GROUP_EMAIL_INVITES`(1회용 토큰 모델), `AI_REVIEW_RUNS`(AI 코드 리뷰 시도 단위), `ASSIGNMENT_COHORT_ANALYSES`/`ASSIGNMENT_COHORT_ANALYSIS_MEMBERS`(집단 비교), `COMMENTS`/`REVIEWS`/`REVIEW_REPLIES`(`is_ai_bot`, `ai_review_run_id` 컬럼 추가), `USERS`(`is_system_bot` + `provider='system'`로 `AI 튜터` 단일 봇 사용자 등록). `SUBMISSION_TRANSLATIONS`는 현재 스키마에 포함하지 않음.
...
- [x] 엔티티 작성. ERD 23개 엔티티 모두 작성(1차). 신 모델 변경 사항은 Phase 4/5/9/10에서 마이그레이션으로 정리합니다.
  ...
  - 신규 또는 변경 대상: `GROUPS`(설명/최대 인원/그룹 코드/가입 토글/그룹 규칙 컬럼), `GROUP_INVITE_LINKS`(영구 토큰), `GROUP_EMAIL_INVITES`(1회용 토큰), `AI_REVIEW_RUNS`(AI 코드 리뷰 시도), `COMMENTS`/`REVIEWS`/`REVIEW_REPLIES`(`is_ai_bot`, `ai_review_run_id` 컬럼), `USERS`(`is_system_bot` + `provider='system'`). (집단 분석 테이블·번역 테이블 제외 여부는 해당 Phase 마이그레이션 기준)
...
- [x] 그룹장 위임 API.
- [x] 그룹 삭제 시 연관 데이터 실삭제 트랜잭션. (`GROUP_INVITE_LINKS`, `GROUP_EMAIL_INVITES` 포함. AI 리뷰 런·집단 분석 등 후속 테이블은 해당 Phase에서 추가 검증)
...
- [x] 그룹 생성 페이지(`/groups/new`). 그룹명/설명/최대 인원수/가입 방식 4 토글/그룹 규칙 6 항목 폼. 헤더 인라인 생성 폼 제거. 모든 라벨/캡션을 `t()` 키로 i18n.
- [-] 그룹 상세 대시보드. (그룹 코드·초대 관리 진입은 반영. 설정 탭의 전체 메타 수정 폼은 아래 `그룹 설정 화면` 항목에서 마무리)
- [x] 그룹 설정 화면. 신 모델 폼(그룹명/설명/최대 인원/가입 토글 4종/그룹 규칙 6항목 + 그룹 코드 갱신 모달). (`GroupDetailClient` 설정 탭 확장)
...
## 10. Phase 5 - 가입 플로우 (영구 그룹 코드 + 영구 초대 링크; 가입 신청·이메일 초대는 제거됨)

### 10.1 진행 방향

- 가입 경로는 `코드`와 `링크` 두 가지입니다.
...
- 2 토글 중 하나라도 꺼지면 해당 경로의 가입 API는 `FORBIDDEN JOIN_DISABLED`로 차단됩니다. 모두 꺼지면 가입 동결.
...
## 14. Phase 9 - 그룹 규칙 단일화 (기계 번역 제외)

### 14.1 진행 방향

- 피드백 공개 정책 매트릭스(역할 × 단위 × deadline 전후) + 과제별 override는 폐기됐습니다. 그룹원 모두 공개로 단일화되었으므로 별도 정책 엔진을 만들지 않습니다.
- 그룹 규칙 6항목(`useDeadline`, `defaultDeadlineTime`, `allowLateSubmission`, `useAiFeedback`, `allowEditAfterSubmit`, `assignmentCreatorRoles`)을 단일 source로 적용합니다.
- 제출 코드 기계 번역 API·`SUBMISSION_TRANSLATIONS`·그룹 규칙 `translationLanguage`는 **현재 범위에서 제외**합니다(`design.md` 5.4.4).

### 14.2 BE

- [ ] 그룹 규칙 적용 단일 함수(`canCreateAssignment(groupRules, actorRole)`, `canEditSubmissionCode(groupRules, isAuthor, ...)`, `canPostLateSubmission(...)` 등).
...
- [x] 그룹 생성/수정 폼의 그룹 규칙 6항목 입력 컨트롤.
...
- 그룹 규칙으로만 과제 생성 가능 역할, 마감일 사용 여부, 지각 허용, AI 피드백, 제출 후 수정이 모두 통제됩니다.
- 같은 그룹원은 누구나 같은 그룹의 모든 제출/댓글/리뷰를 조회할 수 있습니다.
...
- [ ] OpenRouter 클라이언트 모듈(모델별 환경변수 분리: `LLM_MODEL_PROBLEM_ANALYZE`, `LLM_MODEL_SUBMISSION_REVIEW`, 집단 분석 등).
...
- [ ] AI 코드 리뷰 작업.
  ...
  - [ ] 같은 버전에 AI 튜터 댓글·리뷰가 이미 있으면 `CONFLICT`로 중복 트리거 거부.
  - [ ] (선택) `AI_REVIEW_RUNS` 행을 `PENDING/RUNNING/DONE/FAILED`로 관리.
  - [ ] LLM 응답을 우리 시스템에 등록.
  ...
  - [ ] 같은 버전은 AI 피드백 1회만(봇 행이 남아 있으면 재요청 불가; 삭제 후 재요청 가능).
...
- [ ] 과제 집단 코드 비교 분석(`design.md` 5.4.5, `api-spec.md` §11).
  - [x] 집단 번들 `regions` 정책(제출당 1~5·제출별 독립·12줄+ 시 구역 최소 2줄·`whole_file` 금지) 및 리포트 발췌-only 프롬프트(`cohort-analysis-bundle.ts`, `assignment-cohort-analysis.service.ts`).
  ...
  - [x] 마감 전·제출 2 미만·동시 `RUNNING` 등 거절 규칙. 과제당 `DONE` 1회만.
...
- [x] 마감 후 과제 상세(또는 전용 화면)에서 집단 코드 비교 분석 트리거·진행·결과(Markdown + 제출별 원문 코드 열). (가상 파일 `main` 1개 기준)
...
- [ ] E2E 시나리오 1. (Google 로그인 → 그룹 생성(규칙 6개 포함) → 과제 생성 → 코드 제출 → AI 봇 리뷰 등록 확인 → 사람 댓글/리뷰)
...
- [ ] E2E 시나리오 5. (마감 후 집단 코드 비교 분석 트리거·완료·원문 코드 열 표시)
...
- 작업 ID: P3-HOME-001
- 작업 제목: 홈 칸반 보드와 최근 제출 데이터 표시 복구
- 시작 시각: 2026-05-09
- 종료 시각: 2026-05-09
- 변경 파일: `be/src/modules/users/users.controller.ts`, `fe/app/page.tsx`, `fe/app/HomeClient.tsx`, `fe/app/page.module.css`, `fe/app/globals.css`, `fe/src/i18n/messages.ts`, `fe/src/providers/AppProviders.tsx`, `design/design.md`, `design/api-spec.md`, `design/checklist.md`
- 실행 테스트 명령: `pnpm --filter be exec tsc --noEmit`, `pnpm --filter fe exec tsc --noEmit`, `node scripts/verify-i18n.mjs`(cwd `fe`), `pnpm --filter fe build`, `pnpm --filter be test`, `pnpm --filter fe test`, IDE lints
- 테스트 결과: BE/FE TypeScript noEmit 통과, i18n ko/en 503개 키 동기화 통과, FE build 통과(기존 `NewSubmissionForm.module.css` autoprefixer 경고는 남음), BE 121 tests 통과, FE 6 tests 통과, 변경 파일 linter 오류 없음
- 체크리스트 반영 완료 여부: yes
- 핵심 결정
  - 홈은 `해야 할 일`/`한 일`/`최근 알림` 3열 칸반 보드로 구성합니다.
  - `해야 할 일`은 참여 그룹의 과제 중 내 제출이 없는 항목을 마감 임박순으로 계산합니다.
  - `한 일`은 `GET /api/v1/users/me/submissions?limit=5&sort=createdAtDesc`의 최근 제출을 사용합니다.
  - 최근 제출 raw SQL alias는 PostgreSQL 소문자 alias 규칙을 고려해 snake_case로 받습니다.
...
- **그룹 모델 v2** 적용 완료. 영구 그룹 코드 1개, 영구 초대 링크, 1회용 이메일 초대(Resend), 가입 토글 4종, 그룹 규칙 6항목(기계 번역 규칙 제외).
...
- **홈 대시보드는 칸반형 보드로 연결됨.** 최근 알림, 최근 제출, 내 미제출 과제를 서버 컴포넌트에서 조합해 `HomeClient`에 주입합니다.
- **Phase 8(댓글/리뷰), Phase 9(그룹 규칙 일부), Phase 10(AI worker·집단 분석), Phase 11(알림 SSE), Phase 12~14는 부분 완료 또는 미시작.**
...
| 5 | 홈 대시보드 후속 개선 | 현재는 서버 컴포넌트 조합 방식입니다. 과제 수가 많아지면 홈 전용 요약 API로 N+1 제출 조회를 줄입니다. |
| 6 | Phase 8 댓글/리뷰 | `design/checklist.md` §13. |
| 7 | Phase 9 그룹 규칙, Phase 10 AI worker·집단 분석 | §14, §15. AI 봇 사용자(`is_system_bot=true`)는 이미 시드에 들어 있음. |
...
- 작업 ID: INFRA-GCE-001
- 작업 제목: 비용 최소화 GCE 단일 VM 배포 파일 추가
- 시작 시각: 2026-05-11
- 종료 시각: 2026-05-11
- 변경 파일: `.dockerignore`, `docker-compose.prod.yml`, `Caddyfile`, `fe/Dockerfile`, `be/Dockerfile`, `worker/Dockerfile`, `.github/workflows/deploy.yml`, `package.json`, `design/checklist.md`
- 실행 테스트 명령: `APP_ENV_FILE`·`IMAGE_REGISTRY`·`IMAGE_TAG`를 둔 상태에서 `docker compose -f docker-compose.prod.yml config --quiet`, `pnpm --filter be typecheck`, `pnpm --filter worker test`, `NEXT_PUBLIC_API_BASE_URL=/api/v1 pnpm --filter fe build`, `docker build -f be/Dockerfile -t psstudio-be:test .`, `docker build -f worker/Dockerfile -t psstudio-worker:test .`, `docker build -f fe/Dockerfile --build-arg NEXT_PUBLIC_API_BASE_URL=/api/v1 -t psstudio-fe:test .`
- 테스트 결과: compose config 통과, BE typecheck 통과, worker 1 test 통과, FE build 통과(기존 `NewSubmissionForm.module.css` autoprefixer 경고 유지), BE/Worker/FE Docker 이미지 빌드 통과
- 체크리스트 반영 완료 여부: yes
- 핵심 결정
  - 운영 배포는 GCE 단일 VM과 `docker-compose.prod.yml`을 따르며, 설계 요약은 `design/architecture.md` 3.4.1을 본다.
  - Caddy가 `/`는 FE, `/api/*`는 BE로 라우팅하고 Redis는 VM 내부 컨테이너로만 노출한다.
  - GHCR 이미지(`IMAGE_REGISTRY`/`IMAGE_TAG`)를 빌드·푸시한 뒤 SSH로 VM에서 `IMAGE_REGISTRY`·`IMAGE_TAG`·`APP_ENV_FILE`을 넘겨 `docker compose pull && docker compose up -d`를 실행한다.

---

## 25. 랜딩 UI 동기화·스타일 분리(2026-05-11)

**스펙 매핑(랜딩 섹션 → 실제 화면 참고, 앱 라우트 TS/CSS는 수정 금지).**

- [x] 히어로 칸반 `MiniHomeKanban` → `fe/app/HomeClient.tsx` + `fe/app/page.module.css` 동일 규칙(`fe/app/landing/mock/home/HomeDashboardMock.module.css` 복제).
- [x] 히어로 알림 `MiniNotifyList` → `fe/app/notifications/notifications.module.css` 복제본 `mock/notifications/NotificationsListMock.module.css`.
- [x] 그룹 스트립 `MiniGroupsStrip` → `fe/app/groups/explore/page.module.css` 복제본 `mock/groups/GroupsExploreMock.module.css`.
- [x] 과제 쇼케이스 `MiniAssignmentShowcase` → `fe/src/assignments/AssignmentList.module.css` 복제본.
- [x] 캘린더 `MiniCalendar` → 그룹 캘린더 `page.module.css` 복제본 `mock/calendar/GroupCalendarMock.module.css`.
- [x] Diff·리뷰 AI `MiniMergedCodeReviewAi` → `DiffViewerClient.module.css` 복제본 + 인라인 코멘트 UI는 실제 `DiffViewerClient` FragmentRow 폼 구조(Write/Preview, `formActions`, `submission.diff.*` 문자열)에 맞춤.
- [x] 코호트 `MiniCohortReportShowcase` → `CohortAnalysisClient.module.css` 복제본 + 기존 `CohortReportBody`/`CohortCodeColumns` 유지.
- [x] 댓글 카드 목업 → `CommentCard.module.css` 복제본 `mock/comments/CommentCardMock.module.css`.
- [x] 구현 위치: `fe/app/landing/mock/LandingVisualMockupsImpl.tsx`, 루트 `LandingVisualMockups.tsx`는 re-export만.
- [x] 테마: `landing.module.css`, `LandingVisualMockups.module.css` 전역 토큰화, 불필요한 `:global(html[data-theme="dark"])` 제거.
- [x] SEO·JSON-LD: `fe/app/landing/page.tsx`에서 `psstudio.locale` 쿠키 기반 locale.
