...
- `POST /assignments/:assignmentId/cohort-analysis`
  ...
  - 동작: 비동기 파이프라인 실행. 파이프라인 시작 시 과제 `problem_url`에서 위와 동일한 규칙으로 문제 본문을 추출해 LLM 입력 JSON에 `problemContext`(실패 시 `null`)·`problemUrl`을 포함한다. 각 제출 원문은 **`lines` 문자열 배열**로 넘겨 regions 줄 번호와 인덱스를 일치시킨다. 리포트는 문제 요구와 제출 간 교차 비교를 서술하도록 프롬프트한다.
