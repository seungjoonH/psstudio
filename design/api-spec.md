...
- 집단 분석 LLM **시스템 프롬프트**에는 저장소 루트 `design/cohort-report-template.example.md` 전문을 **참고 본문**으로 삽입해 비교 서술·`[[SUBMISSION:uuid]]`·표 없는 나열 방식·펜스 배치 품질을 맞춘다. 해당 파일의 목차 문구·순서·`###` 소제목 체계는 문체 샘플일 뿐이며 **동일 목차 복붙을 요구하지 않는다.** 실제 `reportMarkdown`의 비교 축 제목은 **`##` 비교 주제 + (선택) 백틱 대표 `roleId`** 규약이 우선한다(프롬프트에 명시).
