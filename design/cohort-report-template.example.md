<!--
  ...
  - 아래 목차(전처리·주요 반복문 등)는 문체 샘플일 뿐이다. 실제 분석에서는 제출마다 반복문 유무·구역 이름이 다를 수 있다. 자동 파이프라인에서는 개요에서 연 축 순서와 JSON regions·각 절 펜스가 같은 기준으로 맞도록 프롬프트가 요구한다.
...
다음은 각 제출이 어떤 식으로 반복문을 사용했는지에 대한 요약입니다. 제출이 여러 건일 때도 **표는 쓰지 않고**, 제출마다 같은 필드 순서(순회 방식 → 갱신·판정 요약)로 **소제목과 짧은 문단**으로 나란히 비교합니다.
