<!--
  ...
  - **LLM 출력 주의:** 이 파일의 본문 문장·불릿 라벨·절 구성은 **복사 금지.** 의도는 「비교 리포트가 이런 밀도로 읽히면 된다」는 감각만 전달한다. 실제 생성 시에는 INPUT 코드만 보고 **처음부터 새로** 쓴다. 예시와 같은 두 줄 불릿 틀을 제출마다 반복 채우기 금지.
...
세 제출 모두 요일과 사람(또는 일)을 도는 반복을 쓰지만, 안쪽에서 무엇을 갱신하는지가 갈립니다.

#### [[SUBMISSION:aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa]]

직원 바깥·요일 안쪽 이중 `for` 안에서 출근 스트릭과 지각 여부를 바로 반영하는 형태입니다.

#### [[SUBMISSION:bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb]]

사람과 요일을 도는 이중 루프는 비슷하지만, 먼저 날짜별 지각 여부를 쌓아 두었다가 주 단위로 한 번에 판정합니다.

#### [[SUBMISSION:cccccccc-cccc-4ccc-8ccc-cccccccccccc]]

같은 문제를 일 단위 단일 `for`로 돌려 소비하는 쪽이라, 루프 한 바퀴가 의미하는 시간 단위가 위 두 제출과 다릅니다.

[[SUBMISSION:bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb]]에서는 이중 루프 안에서 **불리언 격자**를 직접 채우는 방식도 함께 씁니다.
